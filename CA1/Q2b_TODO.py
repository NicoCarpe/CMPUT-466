...
def predict(X, w, y=None):
    ...
    loss = 1/(2*X.shape[0]) * np.dot((y_hat - y).T, (y_hat - y))[0][0]
...
def train(X_train, y_train, X_val, y_val):
    ...
    for epoch in range(1, MaxIter + 1):
        print(f"Epoch {epoch}")
        # randomly choose a decay value for the epoch
        epoch_decay = np.random.choice(decay) 
        ...
        for b in range(int(np.ceil(N_train/batch_size))):
            ...
            w = w - alpha*((1/batch_size) * np.dot((X_batch).T, (y_hat_batch - y_batch)) + (epoch_decay * w))
        ...
        if risk < risk_best:
            ...
            w_best = w
            epoch_best = epoch 
            decay_best = epoch_decay

    # Return some variables as needed
    return w_best, risk_best, epoch_best, losses_train, risks_val, decay_best
...
#extend X by its point-wise quadratic features
...
# TODO: Your code here
w_best, risk_val_best, epoch_best, losses_train, risks_val, decay_best = train(X_train, y_train, X_val, y_val)
...
print(f"Decay with best validation performance : decay {decay_best}")
...
plt.tight_layout()
plt.savefig('Training_Loss_B.jpg')
...
plt.tight_layout()
plt.savefig('Validation_Risk_B.jpg')