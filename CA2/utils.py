...
def softmax(z):    
    ...
    # Calculating softmax (iterate over all of the classes in z)
...
def one_hot(t, K):
    ...
    t_hot = np.zeros((t.shape[0], K))

    # indexes each pair from t and the np.arrange arrays and sets that value to 1
    t_hot[np.arange(t.shape[0]), t] = 1
